...
2. Give you stack a name (ex. dropoff)
3. In the web editor, paste the content of the [docker-compose](https://github.com/antoszka/drop-off/blob/main/docker-compose.yml) file.
...
```bash
git clone https://github.com/antoszka/drop-off
cd drop-off
...
npm install
npm run build
pm2 start --name="drop-off-backend" npm -- run prod
...
pm2 start npm --name "drop-off-frontend" -- run start
```

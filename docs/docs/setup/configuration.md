...
If you use Docker you can create a `config.yml` file based on the [`config.example.yaml`](https://github.com/antoszka/drop-off/blob/main/config.example.yaml) and mount it to `/opt/app/config.yaml` in the container.

If you run Pingvin Share without Docker, you can create a `config.yml` file based on the [`config.example.yaml`](https://github.com/antoszka/drop-off/blob/main/config.example.yaml) in the root directory of the project.
...
| `DATABASE_URL`   | `file:../data/drop-off.db?connection_limit=1` | The URL of the SQLite database.                                                                          |

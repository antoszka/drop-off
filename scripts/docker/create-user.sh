...
if ! getent group drop-off-group > /dev/null 2>&1; then
    addgroup -g "$PGID" drop-off-group
fi

# Check if a user with PUID exists; if not, create it
if ! id -u drop-off > /dev/null 2>&1; then
    if ! getent passwd "$PUID" > /dev/null 2>&1; then
        adduser -u "$PUID" -G drop-off-group drop-off > /dev/null 2>&1
    else
